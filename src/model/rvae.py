...
def build_RVAE(cfg, device="cpu"):
    ...
    dense_x_gx = (
        []
        if cfg.get("Network", "dense_x_gx") == ""
        else [int(i) for i in cfg.get("Network", "dense_x_gx").split(",")]
    )
    ...
    dense_z_gz = (
        []
        if cfg.get("Network", "dense_z_gz") == ""
        else [int(i) for i in cfg.get("Network", "dense_z_gz").split(",")]
    )
    dim_RNN_g_z = cfg.getint("Network", "dim_RNN_g_z")
    num_RNN_g_z = cfg.getint("Network", "num_RNN_g_z")
    dense_g_z = (
        []
        if cfg.get("Network", "dense_g_z") == ""
        else [int(i) for i in cfg.get("Network", "dense_g_z").split(",")]
    )
    # Generation
    dense_z_h = (
        []
        if cfg.get("Network", "dense_z_h") == ""
        else [int(i) for i in cfg.get("Network", "dense_z_h").split(",")]
    )
    ...
    model = RVAE(
        ...
        dim_RNN_g_x=dim_RNN_g_x,
        num_RNN_g_x=num_RNN_g_x,
        bidir_g_x=bidir_g_x,
        dense_z_gz=dense_z_gz,
        dim_RNN_g_z=dim_RNN_g_z,
        num_RNN_g_z=num_RNN_g_z,
        dense_g_z=dense_g_z,
        dense_z_h=dense_z_h,
        dim_RNN_h=dim_RNN_h,
        num_RNN_h=num_RNN_h,
...
class RVAE(nn.Module):
    ...
    def build(self):
            ...
            for n in range(len(self.dense_x_gx)):
                if n == 0:
                    dic_layers["linear" + str(n)] = nn.Linear(
                        self.x_dim, self.dense_x_gx[n]
                    )
                else:
                    dic_layers["linear" + str(n)] = nn.Linear(
                        self.dense_x_gx[n - 1], self.dense_x_gx[n]
                    )
                dic_layers["activation" + str(n)] = self.activation
                dic_layers["dropout" + str(n)] = nn.Dropout(p=self.dropout_p)
        self.mlp_x_gx = nn.Sequential(dic_layers)
        self.rnn_g_x = nn.LSTM(
            dim_x_gx, self.dim_RNN_g_x, self.num_RNN_g_x, bidirectional=self.bidir_g_x
        )
        # 2. z_tm1 to g_t^z
        dic_layers = OrderedDict()
        if len(self.dense_z_gz) == 0:
            ...
            for n in range(len(self.dense_z_gz)):
                if n == 0:
                    dic_layers["linear" + str(n)] = nn.Linear(
                        self.z_dim, self.dense_z_gz[n]
                    )
                else:
                    dic_layers["linear" + str(n)] = nn.Linear(
                        self.dense_z_gz[n - 1], self.dense_z_gz[n]
                    )
                dic_layers["activation" + str(n)] = self.activation
                dic_layers["dropout" + str(n)] = nn.Dropout(p=self.dropout_p)
        ...
        if len(self.dense_g_z) == 0:
            ...
            for n in range(len(self.dense_g_z)):
                if n == 0:
                    dic_layers["linear" + str(n)] = nn.Linear(
                        num_dir_x * self.dim_RNN_g_x + self.dim_RNN_g_z,
                        self.dense_g_z[n],
                    )
                else:
                    dic_layers["linear" + str(n)] = nn.Linear(
                        self.dense_g_z[n - 1], self.dense_g_z[n]
                    )
                dic_layers["activation" + str(n)] = self.activation
                dic_layers["dropout" + str(n)] = nn.Dropout(p=self.dropout_p)
        ...
        if len(self.dense_z_h) == 0:
            ...
            for n in range(len(self.dense_z_h)):
                if n == 0:
                    dic_layers["linear" + str(n)] = nn.Linear(
                        self.z_dim, self.dense_z_h[n]
                    )
                else:
                    dic_layers["linear" + str(n)] = nn.Linear(
                        self.dense_z_h[n - 1], self.dense_z_h[n]
                    )
                dic_layers["activation" + str(n)] = self.activation
                dic_layers["dropout" + str(n)] = nn.Dropout(p=self.dropout_p)
        ...
        # 2. h_t, recurrence
        self.rnn_h = nn.LSTM(
            dim_z_h, self.dim_RNN_h, self.num_RNN_h, bidirectional=self.bidir_h
        )
        ...
        if len(self.dense_h_x) == 0:
            ...
            for n in range(len(self.dense_h_x)):
                if n == 0:
                    dic_layers["linear" + str(n)] = nn.Linear(
                        num_dir_h * self.dim_RNN_h, self.dense_h_x[n]
                    )
                else:
                    dic_layers["linear" + str(n)] = nn.Linear(
    ...
    def inference(self, x, v=None):
        ...
        g_z_t = torch.zeros(self.num_RNN_g_z, batch_size, self.dim_RNN_g_z).to(
            self.device
        )
        c_z_t = torch.zeros(self.num_RNN_g_z, batch_size, self.dim_RNN_g_z).to(
            self.device
        )
        ...
        for t in range(0, seq_len):
            ...
            g_z_t_last = g_z_t.view(self.num_RNN_g_z, 1, batch_size, self.dim_RNN_g_z)[
                -1, :, :, :
            ]
    ...
    def generation_x(self, z, v=None):
        ...
        # 3. h_t to y_t
        hx = self.mlp_h_x(h)
        y = self.gen_logvar(hx)

        return y

    def generation_z(self, Z, v=None):
        z_mean_p = torch.zeros_like(Z)
        z_logvar_p = torch.zeros_like(Z)
...
if __name__ == "__main__":
    ...
    def loss_function(recon_x, x, mu, logvar, mu_prior, logvar_prior):
        ...
        KLD = -0.5 * torch.sum(
            logvar
            - logvar_prior
            - torch.div((logvar.exp() + (mu - mu_prior).pow(2)), logvar_prior.exp())
        )
        return recon + KLD
