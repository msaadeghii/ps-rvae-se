...
class SGLD(Optimizer):
    ...
    def step(self, closure=None, w_n=1.0):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                noise_std = torch.Tensor([2 * group["lr"]]).to(
                    p.device.type
                ) * torch.Tensor([w_n]).to(p.device.type)
...
class pSGLD(Optimizer):
    ...
    def __init__(
        self, params, lr=1e-2, beta=0.99, Lambda=1e-15, weight_decay=0, centered=False
    ):
        if not 0.0 <= lr:
            raise ValueError("Invalid learning rate: {}".format(lr))
        if not 0.0 <= Lambda:
